        ...
        <div class="container-fluid">
            ...
            <div class="card card-default">
                <div class="card-header">
                    <h3 class="card-title">{{ __('New Event Wall') }}</h3>
                    <div class="card-tools">
                        <a href="{{ route('admin.events.index') }}" class="btn btn-sm bg-teal"><i
                                class="fas fa-list-alt"></i> {{ __('Event Walls') }}</a>
                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('admin.events.update', $event->id) }}" method="post" id="userForm"
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select class="form-control select2" style="width: 100%;" name="service_id"
                                                ...
                                                @foreach ($services as $item)
                                                    <option value="{{ $item->id }}" @if ($event->service_id == $item->id) {{ 'selected' }} @endif>
                                                        {{ $item->name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        ...
                                        <div class="form-group">
                                            <div class="col-md-12">
                                                ...
                                                <select name="year" id="year" class="form-control select2"
                                                    ...
                                                    @for ($i = $year; $i >= 1971; $i--)
                                                        <option value="{{ $i }}" @if ($event->year == $i) {{ 'selected' }} @endif>
                                                            {{ $i }}</option>
                                                    @endfor
